...
angular.module('angular-confirm', ['ui.bootstrap'])
  .controller('ConfirmModalController', ['$scope', '$modalInstance', 'data', function ($scope, $modalInstance, data) {
      ...
      $scope.ok = function () {
          ...
          //Google Analytics 
          ga('send', 'event', 'New Enquiry', 'Added New enquiry Manually', 'by ' + userCode);
          $modalInstance.close();
      };
  ...
  .value('$confirmModalDefaults', {
      ...
      '<div class="modal-body no-padding"><div class="well no-margin"><h4>{{data.text}}</h4></div></div>' +
      '<div class="modal-footer">' +
      '<button class="btn btn-success btn-sm" ng-click="ok()">{{data.ok}}</button>' +
      '<button class="btn btn-default  btn-sm" ng-click="cancel()">{{data.cancel}}</button>' +
      '</div>',
              ...
              element.unbind("click").bind("click", function ($event) {
                  ...
                  if (angular.isUndefined(scope.confirmIf) || scope.confirmIf) {
                      ...
                      $confirm(data, scope.confirmSettings || {}).then(function () {
                          window.location.href = '#/Details-Tab/0';
                          scope.ngClick(); });
